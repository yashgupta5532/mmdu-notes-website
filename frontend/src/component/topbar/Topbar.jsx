  ...
  return (
    <>
      <div className="topbar">
        <div className="topbar-left">
          ...
          <Link to={`/`} style={{ textDecoration: "none",width:"50%",justifyContent:"center"}} className="topbar-right-Img-left-link">
            <img src={(user && user.profilePicture)?user.profilePicture:profile} className="topbar-right-Img-left" onClick={MenuClickHandler}  />
          </Link>
        </div>
     ...
     <Link to={user ? `/profile/${user._id}` : `/`} style={{ textDecoration: "none" }}>
            <img src={(user && user.profilePicture)?user.profilePicture:profile} className="topbar-menu-Img" />
          </Link>
     </div>
